...
def FMT(a, b, M, N, K):
    ...
    mod = SourceModule("""
    #define BLK_SIZE 32
    __global__ void fmt(double* A, double* B , double* C, int M, int N, int K){
    ...
    fused_matmul_t = mod.get_function("fmt")
    #u_M = np.uint32(M)
    #u_N = np.uint32(N)
    #u_K = np.uint32(K)
...
b_cpu = np.random.randn(N, K).astype(np.float64)
