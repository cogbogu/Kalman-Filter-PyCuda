...
def matrixSub(a, b, M, N):
    ...
    matrixsub = mod.get_function("matSub")
    #u_M = np.uint32(M)
    #u_N = np.uint32(N)
    #u_K = np.uint32(K)
    ...
    matrixsub(
        ...
        grid=(int(math.ceil(float(M / BLK_SIZE))),
              int(math.ceil(float(N / BLK_SIZE)))),
...
c_cpu = np.subtract(a_cpu, b_cpu)

# transfer host (CPU) memory to device (GPU) memory
a_gpu = gpuarray.to_gpu(a_cpu)
b_gpu = gpuarray.to_gpu(b_cpu)
c_gpu = matrixSub(a_gpu, b_gpu, M, N)
